...
then
	...
	sed -i 's/^\(newrelic\.appname = "\).*\("\)/\1API ('$APP_ENVIRONMENT_DISPLAY')\2 /' /usr/local/etc/php/conf.d/newrelic.ini
	sed -i 's/^;\(newrelic\.labels = "\).*\("\)/\1Environment:'$APP_ENVIRONMENT_DISPLAY'\2 /' /usr/local/etc/php/conf.d/newrelic.ini
